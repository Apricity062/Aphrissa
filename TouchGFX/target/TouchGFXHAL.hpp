...
public:
  /**
   * @fn TouchGFXHAL::TouchGFXHAL(touchgfx::DMA_Interface& dma, touchgfx::LCD&
   * display, touchgfx::TouchController& tc, uint16_t width, uint16_t height) :
   ...
   *
   * @brief This function is called whenever the framework has performed a
   * complete draw.
   *
   * This specialization is only in place to keep compilers happy. Base impl.
   * will call the Rect version.
   ...
   * partial draw.
   *
   *        This function is called whenever the framework has performed a
   * partial draw.
   *
  ...
  /**
   * @fn virtual bool TouchGFXHAL::blockCopy(void* RESTRICT dest, const void*
   * RESTRICT src, uint32_t numBytes);
   *
   * @brief This function performs a platform-specific memcpy.
   *
   *        This function performs a platform-specific memcpy, if supported by
   * the hardware.
   *
...
protected:
   ...
   *
   * @return The address of the frame buffer currently being displayed on the
   * TFT.
   */
