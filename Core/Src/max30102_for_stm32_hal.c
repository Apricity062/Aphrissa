...
{
    uint8_t payload[8]; /* 寄存器地址 + 数据(本驱动最大 buflen=3), 用栈缓冲避免 malloc */
    payload[0] = reg;
 ...
 * @param obj Pointer to max30102_t object instance.
 */
int8_t max30102_read_fifo(max30102_t *obj)
{
    ...
    if (num_samples < 0)
    {
        num_samples += 32; /* 写指针回绕时补正 */
    }
    /* num_samples == 0 → FIFO 空(读写指针相等), 返回0不读 */
